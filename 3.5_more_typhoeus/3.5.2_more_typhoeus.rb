...
# add the tab ids you'd like to loop through as an array eg: tab_id = ['a', 'b', 'c', 'd', 'e', 'f'] 
tab_ids = ['assignments', 'people', 'pages' , 'files', 'syllabus', 'outcomes', 'quizzes', 'conferences', 'collaborations']  

CSV.foreach(csv_file, headers: true) do |row|
    sis_course_id = row['course_id']

    tab_ids.each do |tab_id| # loop through each tab_id
